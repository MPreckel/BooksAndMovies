...
type BookToRead = Database['public']['Tables']['books_to_read']['Row']
type BookToReadInsert = Database['public']['Tables']['books_to_read']['Insert']

export function useBooksToRead() {
  ...
  const addBook = async (bookData: Omit<BookToReadInsert, 'user_id' | 'id' | 'created_at' | 'added_date'>) => {
    if (!user) {
  ...
  const isBookInList = (googleBooksId: string) => {
